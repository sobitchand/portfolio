  ...
  useEffect(() => {
    ...
    const initScene = () => {
      ...
      for (let i = 0; i < 1400; i++) {
        const ty = Math.random();               // 0 = hip, 1 = shoulder
        const halfW = 0.42 * (1 - ty * 0.18);  // taper toward top
      ...
      // ── Subtle outer halo (very sparse, large particles) ─────────────────
      for (let i = 0; i < 260; i++) {
        const u = Math.random() * Math.PI * 2;
        const v = Math.acos(2 * Math.random() - 1);
      ...
      // ── Halo glow layer (separate, lower opacity) ─────────────────────────
      ...
      // ── Red orbiting light (visual helper — adds reddish cast via fog) ──
      const redLight = new THREE.PointLight(0xff2200, 2.5, 10);
      scene.add(redLight);

      // Subtle red ambient fog gives the whole scene a blood-red tint
      ...
      const animate = () => {
        ...
        // Figure: slow self-rotation + mouse lean
        figure.rotation.y = t * 0.25 + mouseX * 0.18;
        figure.rotation.x = mouseY * 0.07;
        halo.rotation.y = figure.rotation.y;
        halo.rotation.x = figure.rotation.x;

        // Gentle float: side-to-side + up-down
        const floatY = Math.sin(t * 0.65) * 0.07;
        const floatX = Math.sin(t * 0.43) * 0.04;
        figure.position.y = -0.05 + floatY;
        figure.position.x = floatX;
        halo.position.y = figure.position.y;
        halo.position.x = figure.position.x;

        // Red light orbit (elliptical path around figure)
        const lr = 2.8;
        redLight.position.x = Math.cos(t * 0.38) * lr;
        redLight.position.y = Math.sin(t * 0.29) * 1.4 + 0.3;
        redLight.position.z = Math.sin(t * 0.38) * lr * 0.6 + 3.5;

        // Pulse light intensity for breathing effect
        redLight.intensity = 2.0 + Math.sin(t * 1.1) * 0.6;
